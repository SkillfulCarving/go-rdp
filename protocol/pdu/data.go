...
func (f *FastPathBitmapUpdateDataPDU) Unpack(r io.Reader) error {
	...
	for i := 0; i < int(f.NumberRectangles); i++ {
		...
		if rect.IsCompress() {
			rect.BitmapDataStream = core.Decompress(rect.BitmapDataStream, int(rect.Width), int(rect.Height), Bpp(rect.BitsPerPixel))
		}
		// 对rect 进行 rgb 加载
		n := 0
		pixel := Bpp(rect.BitsPerPixel)
		for y := 0; y < int(rect.Height); y++ {
			for x := 0; x < int(rect.Width); x++ {
				r1, g1, b1, a1 := ToRGBA(pixel, n, rect.BitmapDataStream)
				c := color.RGBA{r1, g1, b1, a1}
				f.Rectangles = append(f.Rectangles, RgbColor{X: int(rect.DestLeft) + x, Y: int(rect.DestTop) + y, Color: c})
				n += pixel
			}
		}
	}
