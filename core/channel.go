...
func NewChannels(t Transport) *Channels {
	c := &Channels{
...
func (c *Channels) SetChannelSender(f ChannelSender) {
	c.channelSender = f
}
...
func (c *Channels) SendToChannel(channel string, s []byte) (int, error) {
	...
	for ln > 0 {
		...
		ln -= len(ss)
		b.Reset()
		WriteUInt32LE(uint32(len(s)), b)
		WriteUInt32LE(flag, b)
		b.Write(ss)
